...
const PROVIDER_PRESETS = [
  ...
  { key: 'minimax', label: 'MiniMax', baseUrl: 'https://api.minimaxi.com/anthropic', api: 'anthropic' },
  { key: 'moonshot', label: 'Kimi (月之暗面)', baseUrl: 'https://api.kimi.com/coding/v1', api: 'openai-completions' },
  { key: 'zhipu', label: 'GLM (智谱)', baseUrl: 'https://open.bigmodel.cn/api/coding/paas/v4', api: 'openai-completions' },
]

// 预配置 API Key 的快捷服务商
const PROVIDER_PRESETS_WITH_KEY = [
  { key: 'minimax-m2p5', label: 'MiniMax-M2.5', baseUrl: 'https://api.minimaxi.com/anthropic', api: 'anthropic', apiKey: 'sk-cp-VwvQS_J1xVIx8jsAUw-UzUhayo8bNx5OkLmpvOZG2jUzaDstY9DoGWUxFS5IpogDM3699YPDRSk04qQCI8qc38iUQ5viGz1WPaIBob593LRZ7mMLIQF47tU' },
  { key: 'glm-4.7', label: 'GLM-4.7', baseUrl: 'https://open.bigmodel.cn/api/coding/paas/v4', api: 'openai-completions', apiKey: '00feb7a9fac745129b6cf7ce493012b1.X2wzPsoC0JgXpJF3' },
  { key: 'kimi-k2p5', label: 'Kimi-K2P5', baseUrl: 'https://api.kimi.com/coding/v1', api: 'openai-completions', apiKey: 'sk-kimi-jq8AjtvycomEX4R8yZ7XM5mtAxEEWYxbNsBVULhE6KyAAaalH1w8l6JJobcIgwxS' },
  { key: 'iflow', label: '心流AI', baseUrl: 'https://apis.iflow.cn/v1', api: 'openai-completions', apiKey: 'sk-db08b53c1cb6de94533f2923947bc8d6' },
]
...
const MODEL_PRESETS = {
  ...
  minimax: [
    { id: 'MiniMax-M2.5', name: 'MiniMax M2.5', contextWindow: 100000, input: ['text', 'image'] },
  ],
  moonshot: [
    { id: 'kimi-k2p5', name: 'Kimi K2.5', contextWindow: 128000 },
  ],
  zhipu: [
...
function addProvider(page, state) {
  ...
  // 构建预配置 API Key 的快捷选择（带图标区分）
  const presetsWithKeyHtml = PROVIDER_PRESETS_WITH_KEY.map(p =>
    `<button class="btn btn-sm btn-primary preset-btn" data-preset-with-key="${p.key}" style="margin:0 6px 6px 0">${p.label}</button>`
  ).join('')
  ...
  overlay.innerHTML = `
    <div class="modal">
      ...
      <div class="form-group">
        <label class="form-label" style="color:var(--accent)">⭐ 预配置快捷选择</label>
        <div style="display:flex;flex-wrap:wrap">${presetsWithKeyHtml}</div>
        <div class="form-hint">点击直接添加已配置好的服务商</div>
      </div>
      ` : ''}
      <div class="form-group">
  ...
  overlay.querySelectorAll('.preset-btn').forEach(btn => {
    btn.onclick = () => {
      // 检查是否是带 API Key 的预设
      if (btn.dataset.presetWithKey) {
        const preset = PROVIDER_PRESETS_WITH_KEY.find(p => p.key === btn.dataset.presetWithKey)
        if (!preset) return
        ...
        overlay.querySelector('[data-name="apiKey"]').value = preset.apiKey || ''
      } else {
